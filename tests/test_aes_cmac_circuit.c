 ...
 *
 * test_aes_cmac_circuit.c - Tests for the AES-CMAC Boolean circuit
 *
 * Tests:
 ...
 *   5: AES-128-CMAC AND gate count       (n+1 AES calls × 7,200)
 *   6: AES-256-CMAC matches software reference
 *   7: AES-128-CMAC NIST CAVP vectors   (Mlen=10,20 K2 path; Mlen=32,33 K1/K2)
 *   8: AES-256-CMAC NIST CAVP vectors   (Mlen=10,15 K2 path; Mlen=16,48 K1 path)
 */
 ...
 *
 * Matches the circuit's byte/bit convention exactly:
 *   - Bytes in natural order, bit 0 = LSB of each byte.
 *   - Shift left operates on the block as a big-endian integer
 *     (MSB = bit 7 of byte 0).
 * ================================================================ */
         ...
         uint8_t tag[16])
{
    ...
    /* K1 = (L << 1) XOR (MSB(L) ? 0x87 at byte 15 : 0) */
    uint8_t K1[16], K2[16];
    uint8_t msb;
...
/*
 * Build a circuit, evaluate it, and return the 16-byte CMAC tag.
 *
 * voleith_circuit_eval() indexes witness/instance arrays by sequential
 * bit position among wires of that kind, in circuit-construction order.
 * Since we add key bits then message bits in byte-then-LSB-first-bit order,
 * the raw key and message byte arrays can be passed directly.
 */
static void
eval_cmac_circuit(const uint8_t *key, int key_bits, const uint8_t *msg,
                  size_t msg_bytes, uint8_t result[16])
{
    ...
    wire_id *msg_wires = NULL;
    if (msg_bytes > 0) {
        msg_wires = calloc(msg_bytes * 8, sizeof(wire_id));
        if (!msg_wires) {
            voleith_circuit_free(c);
            memset(result, 0, 16);
            return;
        }
        for (size_t i = 0; i < msg_bytes * 8; i++)
            msg_wires[i] = voleith_circuit_add_instance(c);
    }

    wire_id tag_wires[128];
    aes_cmac_circuit(c, key_wires, (size_t)key_bits, msg_wires, msg_bytes * 8,
                     tag_wires);
    ...
    voleith_circuit_eval(c, key, msg_bytes > 0 ? msg : NULL, vals);

    for (int i = 0; i < 16; i++) {
        result[i] = 0;
        for (int bit = 0; bit < 8; bit++) {
            wire_id w = tag_wires[i * 8 + bit];
            result[i] |= (uint8_t)(((vals[w / 8] >> (w % 8)) & 1) << bit);
        }
    }
    free(vals);
    free(msg_wires);
...
/* ================================================================
 * Test 1-4: AES-128-CMAC vs RFC 4493 known-answer vectors
 *
...
test_cmac_rfc4493(void)
{
    ...
    /* Example 1: Mlen = 0 */
    {
        static const uint8_t expected[16] = {0xbb, 0x1d, 0x69, 0x29, 0xe9, 0x59,
                                             ...
                                             0x9b, 0x75, 0x67, 0x46};
        uint8_t result[16];
        eval_cmac_circuit(K, 128, NULL, 0, result);
        check("AES-128-CMAC empty message (RFC 4493 Ex.1)",
    ...
    /* Example 2: Mlen = 16 bytes */
    {
        ...
        static const uint8_t expected[16] = {0x07, 0x0a, 0x16, 0xb4, 0x6b, 0x4d,
                                             ...
                                             0xd0, 0x4a, 0x28, 0x7c};
        uint8_t result[16];
        eval_cmac_circuit(K, 128, M, 16, result);
        check("AES-128-CMAC 16 bytes (RFC 4493 Ex.2)",
              memcmp(result, expected, 16) == 0);
    }
    ...
    /* Example 3: Mlen = 40 bytes */
    {
        ...
        static const uint8_t expected[16] = {0xdf, 0xa6, 0x67, 0x47, 0xde, 0x9a,
                                             ...
                                             0x14, 0x97, 0xc8, 0x27};
        uint8_t result[16];
        eval_cmac_circuit(K, 128, M, 40, result);
        check("AES-128-CMAC 40 bytes (RFC 4493 Ex.3)",
              memcmp(result, expected, 16) == 0);
    }
    ...
    /* Example 4: Mlen = 64 bytes */
    {
        ...
        static const uint8_t expected[16] = {0x51, 0xf0, 0xbe, 0xbf, 0x7e, 0x3b,
                                             ...
                                             0x79, 0x36, 0x3c, 0xfe};
        uint8_t result[16];
        eval_cmac_circuit(K, 128, M, 64, result);
        check("AES-128-CMAC 64 bytes (RFC 4493 Ex.4)",
              memcmp(result, expected, 16) == 0);
    }
}

/* ================================================================
 * Test 5: AND gate counts
 * ================================================================ */

static void
test_and_gate_count(void)
{
    /*
     * AND gates = (n_cbc_blocks + 1) × aes_and_gates
     * where n_cbc_blocks = max(1, ceil(msg_bytes / 16))
     * and aes_and_gates = 7200 for AES-128.
     */

    /* Empty message: 1 subkey AES + 1 CBC AES = 2 × 7200 = 14400 */
    {
        voleith_circuit_t *c = voleith_circuit_new();
        wire_id key[128];
        for (int i = 0; i < 128; i++)
        ...
        wire_id tag[128];
        aes_cmac_circuit(c, key, 128, NULL, 0, tag);
        size_t got = voleith_circuit_and_gate_count(c);
        check("AES-128-CMAC AND gates (empty): 2 × 7200 = 14400", got == 14400);
        voleith_circuit_free(c);
    }

    /* 16-byte message (1 complete block): 2 × 7200 = 14400 */
    {
        voleith_circuit_t *c = voleith_circuit_new();
        wire_id key[128];
        for (int i = 0; i < 128; i++)
        ...
        wire_id msg[128];
        for (int i = 0; i < 128; i++)
            msg[i] = voleith_circuit_add_instance(c);
        wire_id tag[128];
        aes_cmac_circuit(c, key, 128, msg, 128, tag);
        size_t got = voleith_circuit_and_gate_count(c);
        check("AES-128-CMAC AND gates (1 block): 2 × 7200 = 14400",
              got == 14400);
        voleith_circuit_free(c);
    }

    /* 40-byte message (2 complete + 1 partial = 3 CBC calls): 4 × 7200 = 28800 */
    {
        voleith_circuit_t *c = voleith_circuit_new();
        wire_id key[128];
        for (int i = 0; i < 128; i++)
        ...
        for (int i = 0; i < 40 * 8; i++)
            msg[i] = voleith_circuit_add_instance(c);
        wire_id tag[128];
        aes_cmac_circuit(c, key, 128, msg, 40 * 8, tag);
        size_t got = voleith_circuit_and_gate_count(c);
        check("AES-128-CMAC AND gates (40 bytes): 4 × 7200 = 28800",
              got == 28800);
        voleith_circuit_free(c);
    }
}
...
/* ================================================================
 * Test 6: AES-256-CMAC circuit matches software reference
 * ================================================================ */
...
test_cmac_aes256(void)
{
    ...
    eval_cmac_circuit(K, 256, M, 40, result);
    check("AES-256-CMAC circuit matches software reference (40 bytes)",
          memcmp(result, ref, 16) == 0);

    /* Empty message */
    cmac_ref(K, 256, NULL, 0, ref);
    eval_cmac_circuit(K, 256, NULL, 0, result);
    check("AES-256-CMAC circuit matches software reference (empty)",
          memcmp(result, ref, 16) == 0);
}

/* ================================================================
 * Test 7: AES-128-CMAC vs NIST CAVP vectors
 *
 * Covers structural cases not in RFC 4493:
...
test_cmac_cavp_aes128(void)
{
    ...
    /* Mlen=10: K2 path, partial first block (VerAES128 Count=122) */
    {
        ...
        static const uint8_t expected[8] = {0x53, 0x70, 0x2f, 0xa9,
                                            0x8e, 0x6f, 0x9a, 0x19};
        eval_cmac_circuit(K, 128, M, 10, result);
        check("AES-128-CMAC 10 bytes K2 (CAVP VerAES128 #122)",
    ...
    /* Mlen=20: K2 path, 1 full + 4-byte partial (VerAES128 Count=164) */
    {
        ...
        static const uint8_t expected[8] = {0x1d, 0x44, 0x12, 0x8c,
                                            0x3d, 0xb0, 0xf7, 0xb9};
        eval_cmac_circuit(K, 128, M, 20, result);
        check("AES-128-CMAC 20 bytes K2 (CAVP VerAES128 #164)",
              memcmp(result, expected, 8) == 0);
    }
    ...
    /* Mlen=32: K1 path, 2 exact blocks (GenAES128 Count=40) */
    {
        ...
        eval_cmac_circuit(K, 128, M, 32, result);
        check("AES-128-CMAC 32 bytes K1 (CAVP GenAES128 #40)",
    ...
    /* Mlen=33: K2 path, 2 full + 1-byte partial (GenAES128 Count=56) */
    {
        ...
        eval_cmac_circuit(K, 128, M, 33, result);
        check("AES-128-CMAC 33 bytes K2 (CAVP GenAES128 #56)",
              memcmp(result, expected, 15) == 0);
    }
}

/* ================================================================
 * Test 8: AES-256-CMAC vs NIST CAVP vectors
 *
...
test_cmac_cavp_aes256(void)
{
    ...
    /* Mlen=10: K2 path, partial first block (GenAES256 Count=56) */
    {
        ...
        eval_cmac_circuit(K, 256, M, 10, result);
        check("AES-256-CMAC 10 bytes K2 (CAVP GenAES256 #56)",
    ...
    /* Mlen=15: K2 path, maximal partial first block (GenAES256 Count=72) */
    {
        ...
        eval_cmac_circuit(K, 256, M, 15, result);
        check("AES-256-CMAC 15 bytes K2 (CAVP GenAES256 #72)",
              memcmp(result, expected, 10) == 0);
    }
    ...
    /* Mlen=16: K1 path, 1 exact block (GenAES256 Count=24) */
    {
        ...
        eval_cmac_circuit(K, 256, M, 16, result);
        check("AES-256-CMAC 16 bytes K1 (CAVP GenAES256 #24)",
              memcmp(result, expected, 10) == 0);
    }
    ...
    /* Mlen=48: K1 path, 3 exact blocks (GenAES256 Count=40) */
    {
        ...
        eval_cmac_circuit(K, 256, M, 48, result);
        check("AES-256-CMAC 48 bytes K1 (CAVP GenAES256 #40)",
              memcmp(result, expected, 10) == 0);
    }
}
...
main(void)
{
    printf("=== test_aes_cmac_circuit ===\n");

    printf("\n[AES-128-CMAC RFC 4493 known-answer vectors]\n");
    test_cmac_rfc4493();

    printf("\n[AES-128-CMAC AND gate counts]\n");
    test_and_gate_count();

    printf("\n[AES-256-CMAC software reference comparison]\n");
    test_cmac_aes256();

    printf("\n[AES-128-CMAC NIST CAVP vectors]\n");
    test_cmac_cavp_aes128();

    printf("\n[AES-256-CMAC NIST CAVP vectors]\n");
